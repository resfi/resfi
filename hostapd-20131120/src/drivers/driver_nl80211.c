				      ...
				      struct hostapd_sta_add_params *params)
{
	...
	if (!msg)
		return -ENOMEM;
	wpa_printf(MSG_INFO, "nl80211: %s STA " MACSTR,
	...
	NLA_PUT(msg, NL80211_ATTR_STA_SUPPORTED_RATES, params->supp_rates_len,
		params->supp_rates);
	wpa_hexdump(MSG_INFO, "  * supported rates", params->supp_rates,
	...
	if (!params->set) {
		if (params->aid) {
			 ...
			 */
			wpa_printf(MSG_INFO, "  * aid=1 (TDLS workaround)");
			NLA_PUT_U16(msg, NL80211_ATTR_STA_AID, 1);
		}
		wpa_printf(MSG_INFO, "  * listen_interval=%u",
			   params->listen_interval);
		NLA_PUT_U16(msg, NL80211_ATTR_STA_LISTEN_INTERVAL,
			    params->listen_interval);
	} else if (params->aid && (params->flags & WPA_STA_TDLS_PEER)) {
		wpa_printf(MSG_INFO, "  * peer_aid=%u", params->aid);
	...
	wpa_printf(MSG_INFO, "  * capability=0x%x", params->capability);
	...
	upd.set = upd.mask;
	wpa_printf(MSG_INFO, "  * flags set=0x%x mask=0x%x",
		   upd.set, upd.mask);
	...
	if (params->flags & WPA_STA_WMM) {
		...
		if (!wme)
			goto nla_put_failure;
		wpa_printf(MSG_INFO, "  * qosinfo=0x%x", params->qosinfo);
					    ...
					    int flags_or, int flags_and)
{
	...
	if (total_flags & WPA_STA_AUTHORIZED) {
		...
		printf("!!!STA AUTHOIRZED FLAG SET!!!!\n");
	}
